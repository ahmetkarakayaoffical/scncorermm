...
namespace scncore_rmm_agent_health
{
    public class Worker : BackgroundService
    {
        ...
        private async void Check_Health()
        {
            ...
            try
            {
                ...
                {
                    ServiceController sc = new ServiceController("scncore-rmm-agent-comm");
                    if (sc.Status.Equals(ServiceControllerStatus.StartPending) || sc.Status.Equals(ServiceControllerStatus.Running))
                    {
                        Logging.Debug("Check_Health (scncore-rmm-agent-comm)", "Running", "Installed & running.");
                    ...
                    {
                        Logging.Debug("Check_Health (scncore-rmm-agent-comm)", "Stopped|StopPending", "Installed, but stopped or stop is pending.");
                    }

                    sc = new ServiceController("scncore-rmm-agent-remote");
                    if (sc.Status.Equals(ServiceControllerStatus.StartPending) || sc.Status.Equals(ServiceControllerStatus.Running))
                    {
                        Logging.Debug("Check_Health (scncore-rmm-agent-remote)", "Running", "Installed & running.");
                        remote_agent_healthy = true;
                    }
                    else if (sc.Status.Equals(ServiceControllerStatus.Stopped) || sc.Status.Equals(ServiceControllerStatus.StopPending))
                    {
                        Logging.Debug("Check_Health (scncore-rmm-agent-remote)", "Stopped|StopPending", "Installed, but stopped or stop is pending.");
                    }
                }
                else if (OperatingSystem.IsLinux())
                {
                    string[] servicesToCheck = { "scncore-rmm-agent-comm", "scncore-rmm-agent-remote" };
                    ...
                    foreach (var serviceName in servicesToCheck)
                    {
                        ...
                        if (isServiceRunning)
                        {
                            ...
                            if (serviceName == "scncore-rmm-agent-comm")
                                comm_agent_healthy = true;
                            else if (serviceName == "scncore-rmm-agent-remote")
                                remote_agent_healthy = true;
                        }
                        else
                        {
            ...
            if (!comm_agent_healthy || !remote_agent_healthy)
            {
                ...
                try
                {
                    if (OperatingSystem.IsWindows())
                    {
                        ...
                        {
                            sc = new ServiceController("scncore-rmm-agent-comm");
                            sc.Start();
                            failed_count = 0;
                    ...
                    else if (OperatingSystem.IsLinux())
                    {
                        ...
                        {
                            // Start the service
                            Bash.Execute_Script("Service Start", false, "systemctl start scncore-rmm-agent-comm.service");

                            // Check status
                            string output = Bash.Execute_Script("Service status", false, "systemctl list-units --type=service --all | grep -w scncore-rmm-agent-comm.service");
                        ...
                        {
                            // Start the service
                            Bash.Execute_Script("Service Start", false, "systemctl start scncore-rmm-agent-remote.service");
                            
                            // Check status
                            string output = Bash.Execute_Script("Service Sensor", false, "systemctl list-units --type=service --all | grep -w scncore-rmm-agent-remote.service");
                    ...
                    else if (OperatingSystem.IsMacOS())
                    {
                        ...
                        {
                            // Start the service
                            MacOS.Helper.Zsh.Execute_Script("Service Start", false, "launchctl start scncore-rmm-agent-comm");

                            string output = MacOS.Helper.Zsh.Execute_Script("Service Sensor", false, $"launchctl list | grep scncore-rmm-agent-comm");

                            Logging.Debug("Check_Health", "Service status output", output);

                            // Regex to extract only the line for the specific service
                            string pattern = $@"^\S+\s+\S+\s+scncore-rmm-agent-comm$";
                        ...
                        {
                            // Start the service
                            MacOS.Helper.Zsh.Execute_Script("Service Start", false, "launchctl start scncore-rmm-agent-remote");
                         
                            string output = MacOS.Helper.Zsh.Execute_Script("Service Sensor", false, $"launchctl list | grep scncore-rmm-agent-remote");

                            Logging.Debug("Check_Health", "Service status output", output);

                            // Regex to extract only the line for the specific service
                            string pattern = $@"^\S+\s+\S+\s+scncore-rmm-agent-remote$";
                            var match = Regex.Match(output, pattern, RegexOptions.Multiline);
            ...
            if (failed_count == 3 || failed_count > 3)
            {
                ...
                if (!Directory.Exists(Application_Paths.c_temp_scncore_installer_dir))
                    Directory.CreateDirectory(Application_Paths.c_temp_scncore_installer_dir);
                ...
                http_status = await Http.DownloadFileAsync(Global.Configuration.Agent.ssl, update_server + installer_package_url, Path.Combine(Application_Paths.c_temp_scncore_dir, Application_Paths.installer_package_path), Global.Configuration.Agent.package_guid);
                ...
                if (http_status && !String.IsNullOrEmpty(installer_hash) && IO.Get_SHA512(Path.Combine(Application_Paths.c_temp_scncore_dir, Application_Paths.installer_package_path)) == installer_hash)
                {
                    ...
                    try
                    {
                        ZipFile.ExtractToDirectory(Path.Combine(Application_Paths.c_temp_scncore_dir, Application_Paths.installer_package_path), Application_Paths.c_temp_scncore_installer_dir);
                        ...
                        if (OperatingSystem.IsWindows())
                        {
                            ...
                            installer.StartInfo.FileName = Application_Paths.c_temp_scncore_installer_path;
        ...
        private void Delete_Installer()
        {
            ...
            {
                if (Directory.Exists(Application_Paths.c_temp_scncore_installer_dir))
                    Directory.Delete(Application_Paths.c_temp_scncore_installer_dir, true);
            }
            catch (Exception ex)
            {
