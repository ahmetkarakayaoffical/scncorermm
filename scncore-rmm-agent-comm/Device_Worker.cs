        ...
        private async Task Initialize(bool forced)
        {
            try
            {
                ...
                if (communication_server_status)
                {
                    ...
                    if (File.Exists(Application_Paths.program_data_scncore_policy_database) == false || File.Exists(Application_Paths.program_data_scncore_events_database) == false)
                    {
                        Global.Initialization.Database.Scncore_Events_Setup(); //Create events database if its not existing (cause it was deleted somehow)
                        forced = true;
                    }
                    ...
                    if (File.Exists(Application_Paths.just_installed) && forced == false) //Force the sync & set the config because its the first run (justinstalled.txt)
                        forced = true;
                    else if (Windows.Helper.Registry.HKLM_Read_Value(Application_Paths.scncore_reg_path, "Synced") == "0" || Windows.Helper.Registry.HKLM_Read_Value(Application_Paths.scncore_reg_path, "Synced") == null)
                        forced = true;
                    ...
                    if (up2date) // No update required. Continue logic
                    {
                        ...
                        if (authorized && auth_result == "not_synced" || authorized && forced)
                        {
                            ...
                            Windows.Helper.Registry.HKLM_Write_Value(Application_Paths.scncore_reg_path, "Synced", "0");

                            // Sync
                            await Global.Online_Mode.Handler.Policy();

                            // Sync done. Set synced flag in registry to prevent re-sync
                            Windows.Helper.Registry.HKLM_Write_Value(Application_Paths.scncore_reg_path, "Synced", "1");
