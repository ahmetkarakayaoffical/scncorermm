...
namespace scncore_rmm_web_console.Components.Pages.Collections.Scripts
{
    public partial class Manage_Scripts
    {
        ...
        private string scripts_selectedRowContent_id = String.Empty; // Hier wird der Inhalt der ausgew�hlten Zeile gespeichert
        private string scripts_selectedRowContent_json = String.Empty; // Hier wird der Inhalt der ausgew�hlten Zeile gespeichert
        ...
        private async Task Export_Data_Json(string type)
        {
            try
            {
                ...
                // Erstellen eines JSON-Strings aus den MudTable-Eintr�gen
                if (type == "scripts")
                    jsonContent = JsonSerializer.Serialize(scripts_mysql_data, new JsonSerializerOptions { WriteIndented = true });

                // Aufruf der JavaScript-Funktion f�r den Export als .txt
        ...
        public async Task Export_Data_HTML(string type)
        {
            try
            {
                ...
                if (type == "scripts")
                {
                    ...
                    // Baue die Tabelleneintr�ge basierend auf den Daten
