...
using ScncoreRmmWebConsole.Components;
using ScncoreRmmWebConsole;
using System.Net;
using ScncoreRmmWebConsole.Classes.MySQL;
using Blazored.LocalStorage;
using Microsoft.AspNetCore.Components.Authorization;
using Microsoft.AspNetCore.Components.Server.ProtectedBrowserStorage;
using ScncoreRmmWebConsole.Classes.Authentication;
using System.Runtime.InteropServices;
using System.Runtime.Versioning;
using ScncoreRmmWebConsole.Configuration;
using Microsoft.AspNetCore.Hosting.Server;
using Microsoft.Extensions.Configuration;
using Microsoft.AspNetCore.Http.Features;
using ScncoreRmmWebConsole.Components.Pages.Devices;
using LettuceEncrypt;
using LettuceEncrypt.Acme;
using MudBlazor;
using Microsoft.AspNetCore.HttpOverrides;
using static MudBlazor.Defaults;
using ScncoreRmmWebConsole.Classes.Helper;
using Org.BouncyCastle.Asn1.X509.Qualified;

ScncoreRmmWebConsole.Classes.Setup.Directories.Check_Directories(); // Check if directories exist and create them if not
...
var remoteServerConfig = builder.Configuration.GetSection("ScncoreRmm_Remote_Server").Get<ScncoreRmmWebConsole.Classes.Remote_Server.Config>();
...
var fileServerConfig = builder.Configuration.GetSection("ScncoreRmm_File_Server").Get<ScncoreRmmWebConsole.Classes.File_Server.Config>();
...
// Title
Web_Console.title = builder.Configuration.GetValue<string>("Webinterface:Title", "scncore-rmm");

if (Web_Console.title == "Your company name")
    Web_Console.title = "scncore-rmm"; // Default title if not set
...
// Output version
Console.WriteLine("scncore-rmm Web Console");
...
else if (!microsoft_defender_firewall_status && OperatingSystem.IsWindows())
{
    ...
    Console.WriteLine("Microsoft Defender Firewall is disabled. You should enable it for your own safety. scncore-rmm adds firewall rules automatically according to your configuration.");
}
...
else
{
    ...
    else
    {
        ...
        else // Table exists
        {
            ...
            if (String.IsNullOrEmpty(Members_Portal.api_key))
            {
                Members_Portal.api_key = await scncore_rmm_web_console.Classes.MySQL.Handler.Get_Api_Key();

            Console.WriteLine("Members Portal API key loaded from database: " + Members_Portal.api_key);
            }
            else
                await scncore_rmm_web_console.Classes.Members_Portal.Handler.Set_Api_Key(Members_Portal.api_key);

            // Do cloud stuff
            if (Members_Portal.cloud_enabled)
            {
                Console.WriteLine("Cloud enabled. Checking cloud connection...");
                if (await scncore_rmm_web_console.Classes.Members_Portal.Handler.Check_Connection())
                {
                    Console.ForegroundColor = ConsoleColor.Green;
            ...
            // Update license info
            await scncore_rmm_web_console.Classes.Members_Portal.Handler.Request_License_Info_Json(Members_Portal.api_key);
        }
    }
}
...
builder.Services.AddHostedService<ScncoreRmmWebConsole.Classes.MySQL.AutoCleanupService>();
builder.Services.AddHostedService<ScncoreRmmWebConsole.Classes.ScreenRecorder.AutoCleanupService>(); //disabled until remote screen control release
