...
set -e
echo ""
echo "Welcome to the scncore-rmm Docker Quick Setup Script!"
...
echo ""
echo "[ATTENTION!!!]"
echo "The script only covers some scenarious and tries to guide you through the setup process. scncore-rmm supports any environment (single ip, reverse proxies etc.). If you fail with your installation, please read the documentation at: https://docs.scncore-rmm.com/installation/docker-quick-setup"
...
echo ""
echo "Our pro membership includes code signing, unlimited devices and we assist you with the setup of your scncore-rmm instance in your own environment."
echo "We also offer a managed hosting solution for scncore-rmm, please contact us for more information or check our website."
...
if [[ -z "$web_console_title" ]]; then
    echo "No title entered. Defaulting to 'scncore-rmm'."
    web_console_title="scncore-rmm"
fi
...
if [[ "$setup_option" == "1" ]]; then # No Reverse Proxy with Let's Encrypt
    ...
    echo "7443 for the scncore-rmm server (remote access, file transfer, etc.)."
...
echo ""
echo "Thank you for using scncore-rmm!"
